...
export default function PrintSheetPage() {
  ...
  return (
    ...
    >
      ...
      {/* Sheet */}
      <main className="mx-auto max-w-4xl space-y-6 px-6 py-6 text-[12.5px] leading-snug text-leaf-900 print:max-w-none print:space-y-2 print:px-0 print:py-0 print:text-[10px] print:leading-tight">
        <Header bed={bed} />
        ...
        <footer className="mt-6 flex items-center gap-2 border-t border-leaf-200 pt-3 text-[10px] text-leaf-700/60 print:mt-3 print:pt-2">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/joebee.png"
            alt=""
            aria-hidden
            className="h-5 w-5 rounded-full border border-leaf-300 bg-white object-contain p-0.5"
          />
          <span>
            Generated by JoeBees Garden Bed Planner —{" "}
            <span className="font-medium text-leaf-800">joebees.us</span> · printed{" "}
      ...
      <style>{`
        @media print {
          ...
          /* Make sure raster images (the JoeBee logo) print in full color. */
          img {
            -webkit-print-color-adjust: exact !important;
            print-color-adjust: exact !important;
          }
          /* Compact spacing & sizing for 2-page fit. */
          h2 { margin-bottom: 0 !important; }
          table { font-size: 9.5px !important; }
          th, td { padding-top: 1px !important; padding-bottom: 1px !important; }
          ol, ul { margin-top: 2px !important; }
...
function Header({ bed }: { bed: GardenBed }) {
  return (
    <section className="avoid-break flex flex-wrap items-center justify-between gap-3 border-b-2 border-leaf-700 pb-2">
      <div className="flex items-center gap-3">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/joebee.png"
          alt="JoeBees"
          className="h-14 w-14 shrink-0 rounded-full border border-leaf-300 bg-white object-contain p-1 shadow-sm print:shadow-none"
        />
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-leaf-700">
            {bed.width} × {bed.height} ft raised bed · {bed.plants.length} placements · JoeBees Garden Planner
          </p>
        </div>
      </div>
...
/**
 * Floating brand mark on screen — a still frame from the JoeBee intro video,
 * shown in the same bottom-right spot where the "Ask JoeBee" chat button
 * lives on the planner. Hidden when printing (the header logo prints).
 */
function BrandMark() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 z-20 print:hidden">
      <video
        ref={ref}
        src="/JOEBEE.mp4"
        muted
        ...
        preload="auto"
        aria-hidden
        className="h-14 w-14 rounded-full border border-leaf-300 bg-white object-cover shadow-md"
      />
    </div>
  );
}
